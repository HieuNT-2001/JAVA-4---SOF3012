...
public class VideoDAO {
	...
	public static List<Object[]> findTop10Favorited() {
		 String jpql = "SELECT v.id, v.title, v.poster, v.active, COUNT(f.id) " +
                 "FROM Video v JOIN v.favorites f " +
                 "GROUP BY v.id, v.title, v.poster, v.active " +
	...
	public static List<Video> findUnFavorited () {
		String jpql = "SELECT v FROM Video v " +
                "WHERE v NOT IN (SELECT f.video FROM Favorite f)";
		TypedQuery<Video> query = manager.createQuery(jpql, Video.class);
		return query.getResultList();
	}
	
	public static void main(String[] args) {
		System.out.println(VideoDAO.findTop10Favorited());
	}
}
